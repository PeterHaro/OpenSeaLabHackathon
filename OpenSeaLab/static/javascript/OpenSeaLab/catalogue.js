var catalogue = function () {
    ...
    var catalogs = {
        ...
        oscar: CesiumUtility.loadJson([OSCAR_PATH, "catalog.json"].join("/"))
    };
    ...
    function buildProduct(overrides) {
        return _.extend({
            ...
            load: function (cancel) {
                var me = this;
                return when.map(this.paths, CesiumUtility.loadJson).then(function (files) {
    ...
    function gfs1p0degPath(attr, type, surface, level) {
        ...
        var file = [stamp, type, surface, level, "gfs", "1.0"].filter(CesiumUtility.isValue).join("-") + ".json";
    ...
    function describeSurface(attr) {
        return attr.surface === "surface" ? "Surface" : CesiumUtility.capitalize(attr.level);
    }

    function describeSurfaceJa(attr) {
        return attr.surface === "surface" ? "地上" : CesiumUtility.capitalize(attr.level);
    }
                ...
                return buildProduct({
                    ...
                    scale: {
                        ...
                        gradient: function (v, a) {
                            return CesiumUtility.extendedSinebowColor(Math.min(v, 100) / 100, a);
                        }
                    },
        ...
        "temp": {
            ...
            create: function (attr) {
                return buildProduct({
                    ...
                    scale: {
                        bounds: [193, 328],
                        gradient: CesiumUtility.segmentedColorScale([
                            [193, [37, 4, 42]],
                            [206, [41, 10, 130]],
                            [219, [81, 40, 40]],
            ...
            create: function (attr) {
                return buildProduct({
                    ...
                    scale: {
                        bounds: [0, 100],
                        gradient: function (v, a) {
                            return CesiumUtility.sinebowColor(Math.min(v, 100) / 100, a);
                        }
                    }
                });
            }
            ...
            create: function (attr) {
                return buildProduct({
                    ...
                    scale: {
                        bounds: [0, 1.5],
                        gradient: function (v, a) {
                            return CesiumUtility.sinebowColor(Math.min(v, 1.5) / 1.5, a);
                        }
                    }
                });
            }
                        ...
                        builder: function (file) {
                            ...
                            return {
                                ...
                                data: function (i) {
                                    ...
                                    return CesiumUtility.isValue(u) && CesiumUtility.isValue(v) ? [u, v] : null;
                                }
                            }
                        },
                        units: [
    ...
    function lookupOscar(catalog, date, offset) {
        ...
        var prefix = CesiumUtility.ymdRedelimit(date, "/", ""), i = _.sortedIndex(catalog, prefix);
    ...
    function oscarDate(catalog, attr) {
        var file = lookupOscar(catalog, attr.date);
        var parts = file ? CesiumUtility.ymdRedelimit(file, "", "/").split("/") : null;
    ...
    function oscarStep(catalog, date, step) {
        var file = lookupOscar(catalog, CesiumUtility.dateToUTCymd(date, "/"), step > 1 ? 6 : step < -1 ? -6 : step);
        var parts = file ? CesiumUtility.ymdRedelimit(file, "", "/").split("/") : null;
        return parts ? new Date(Date.UTC(+parts[0], parts[1] - 1, +parts[2], 0)) : null;
    }
    ...
    function buildGrid(builder) {
        ...
        function interpolate(λ, φ) {
            var i = CesiumUtility.floorMod(λ - λ0, 360) / Δλ;  // calculate longitude index in wrapped range [0, 360)
            ...
            if ((row = grid[fj])) {
                var g00 = row[fi];
                var g10 = row[ci];
                if (CesiumUtility.isValue(g00) && CesiumUtility.isValue(g10) && (row = grid[cj])) {
                    var g01 = row[fi];
                    var g11 = row[ci];
                    if (CesiumUtility.isValue(g01) && CesiumUtility.isValue(g11)) {
        ...
        return {
            ...
            forEachPoint: function (cb) {
                for (var j = 0; j < nj; j++) {
                    var row = grid[j] || [];
                    for (var i = 0; i < ni; i++) {
                        cb(CesiumUtility.floorMod(180 + λ0 + i * Δλ, 360) - 180, φ0 - j * Δφ, row[i]);
                    }
                }
            }
        };
    ...
    function productsFor(attributes) {
        ...
        return results.filter(CesiumUtility.isValue);
    }

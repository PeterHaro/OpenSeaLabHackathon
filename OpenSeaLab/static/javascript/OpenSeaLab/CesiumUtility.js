var CesiumUtility = function() {
    ...
    /**
     * @returns {Number} cartesianDistance between two points having the form [x, y].
     */
    ...
    function proportion(x, low, high) {
        return (CesiumUtility.clamp(x, low, high) - low) / (high - low);
    }
    ...
    function dateToConfig(date) {
        return {date: CesiumUtility.dateToUTCymd(date, "/"), hour: CesiumUtility.zeroPad(date.getUTCHours(), 2) + "00"};
    }
    ...
    function segmentedColorScale(segments) {
        ...
        return function(point, alpha) {
            ...
            var range = ranges[i];
            return interpolators[i](CesiumUtility.proportion(point, range[0], range[1]), alpha);
        };
    }
     ...
     */
    function formatCoordinates(lambda, phi) {
        return Math.abs(phi).toFixed(2) + "° " + (phi >= 0 ? "N" : "S") + ", " +
            Math.abs(lambda).toFixed(2) + "° " + (lambda >= 0 ? "E" : "W");
    }
